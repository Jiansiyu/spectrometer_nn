---
title: "Spectometer Regression With All Phi Term Multiple Regression Models" 
...
dataSet = dataset_focal %>% #filter(.$runID == 2239 | .$runID == 2240 |  .$runID == 2241) %>%#
...
# ok. let's test on more complicated regression {.tabset}


## ridge regresion

* here only optimize the $\phi$ dimension, keep using the lasso 20-fold regression to save time 

```{r}
#
ridge.fit.phi   = cv.glmnet(as.matrix(train_X), as.matrix(train_y_phi), weights = as.matrix(cv.weight), nfolds = n.fold, alpha = 0)
