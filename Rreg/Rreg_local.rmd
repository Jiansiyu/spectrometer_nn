...
```{r}
dataSet = dataset_focal %>% filter(runID == 2241 | runID == 2240 | runID == 2239) %>% filter(SieveRowID < 5 & SieveRowID > 1)
...
for (index.step  in 1:length(runList) ) {
  ...
  mypt = ggplot(val) +geom_bin2d(aes(x=pred.phi,y=pred.theta,color = "blue"),bins = 300) + geom_point(aes(y=targCalTh,x=targCalPh,color="green")) + 
    xlim(-0.015,0.025) + ylim(-0.045,0.045) + labs(title=sprintf("run_2d_%d",runID.step))
